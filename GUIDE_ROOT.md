...
- [`docker-entrypoint.sh`](/Users/gwh/projects/one-time-projects/podcast-downloader/docker-entrypoint.sh) seeds the mounted data directory with a default `config.ini` and missing state files on first boot, copies image-bundled `.ui_password` and `cookies.txt` into the mounted data path when present, migrates `.ui_password` into a hashed format, then performs a best-effort `yt-dlp` update.
...
- If mounted `cookies.txt` is missing but the image contains `/app/cookies.txt`, the entrypoint copies that file into the mounted data directory on first boot. Existing mounted cookies are never overwritten.
