...
Configured YouTube cookies follow `always_use_cookies` in `config.ini`. When true (default), YouTube `yt-dlp` calls pass the configured Netscape-format cookie file on the first attempt and retry once without cookies on failure. When false, the order is inverted: plain first, cookies on retry. In Docker, the simplest path is putting `cookies.txt` in the mounted data directory or rebuilding with a repo-root `cookies.txt`, which refreshes the mounted data cookie file on startup when contents differ. The file is ignored by git because it contains browser authentication state.
