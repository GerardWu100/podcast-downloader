...
Activity-log timestamps are written in the `America/Toronto` timezone. That keeps the browser activity feed aligned with the operator's local clock even when Docker runs the Python process in Coordinated Universal Time, abbreviated UTC.
