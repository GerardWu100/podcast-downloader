...
The full `download.log` keeps diagnostic detail for debugging. The browser UI defaults to `activity.log`, which contains concise user-facing events such as skipped Shorts, age-gate waits, completed downloads, failures, and run summaries. Both logs use Toronto/Eastern timestamps at minute precision. Operators can switch the log viewer to `download.log` when they need the full runtime tail. The activity log lives beside `download.log` and is derived from that path rather than being a separate config setting.
